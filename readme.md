...
- 内置模型：提供 CNN、GRU/BiGRU 和轻量 Transformer 分类基线；可选接入
  Hugging Face Wav2Vec2、HuBERT、WavLM 等音频模型。
...
- 工程质量：包含跨平台 CI、静态检查、类型检查、分模块覆盖率门禁、构建验证和数据管线基准。
...
| 预训练模型 | Hugging Face Wav2Vec2/HuBERT/WavLM 适配、processor 持久化与离线恢复 | 可选依赖 |
| 训练 | CPU/单 GPU、AMP、类别权重/focal loss、平衡采样、验证、early stopping、训练日志 | 稳定 |
| 评估 | 常用 SER 分类指标和 JSON/JSONL 报告 | 稳定 |
| Artifact | safetensors、完整性校验、模型卡、processor 配置、可移植恢复 | 稳定 |
| 推理 | 单文件、批量和纯 PCM 流式推理 | 稳定核心 |
| CLI | dataset/train/evaluate/predict/artifact 命令 | 稳定核心 |
| 教程 | 0–7 章课程结构 | 提纲，尚不可执行 |

当前 HEAD 的 CI 对 Linux、Windows、macOS 与 Python 3.10–3.12 运行完整测试和 CPU
训练冒烟，并单独验证 Hugging Face 依赖矩阵、静态检查、类型检查、分模块覆盖率以及
源码树外隔离 wheel 安装。此前也已在 RTX 4060 上使用真实 EmotionTalk 数据完成
3 epoch 训练、最佳 checkpoint 导出和独立测试集评估，证明混合采样率、单双声道数据
可贯通训练与 artifact 工作流；该短训结果仅用于工程验收，不代表正式基准。
...
需要 Hugging Face 预训练音频模型时安装正式可选依赖 `hf`：
...
- [核心边界审计](docs/development/review/SER_LIB_CORE_BOUNDARY_AUDIT.md)
