...
- 最终再次比较本分支与 `main`：2026-09-10 复核时 main HEAD 仍为 `7018e05dbbfd5e40207ac8ccfd886cbaedbebfe6`，本分支 ahead 287、behind 0，没有新的 main 提交需要同步。删除的生产代码集中在计划内的 `core`、`services`、跨领域 catalog、旧 pretrained 路径与 detail/preset 应用包装；`data`、`models`、`engine`、`inference`、`artifacts`、`config`、`foundation` 以及 Torch/HF adapters 均继续存在，没有发现计划外整块真实 SER 能力删除。

### Strict review 后续修复

Stage 12 初次收口后又执行了 `docs/development/review/SER_LIB_STRICT_REVIEW_2026-09-10.md` 的逐模块严格审计。该审计是问题发现基线，不因后续修复而改写原结论；本节记录在同一分支上的后续 remediation：
...
- CNN 采用 mask-aware BatchNorm 并在卷积层间清零 padding，使变长输入在不同右侧 padding/batch 组合下保持不变，同时保留既有 BatchNorm state-dict 表面；GRU 同时锁定 lengths 与连续前缀 mask 的一致性。
...
- 高层 `evaluate_artifact()` 暴露 `prediction_sink` 与 `retain_predictions`；关闭预测驻留时只写聚合指标，不覆盖外部流式 sink 已写出的 JSONL，从而把百万级逐样本预测明细内存从 O(N) 降到 O(1)。
- artifact 评估报告显式记录 `metric_unit`：sliding batching 为 `window`，dynamic/fixed 为 `sample`，旧 artifact 无 batching 声明时为 `batch_row`，避免把窗口级指标静默解释为原样本级指标。
- 同步 event callback 契约明确为 fail-fast；`evaluate()` 把首个 `started` callback 也纳入状态清理边界，callback 抛错仍传播，但模型原始 train/eval mode 必须恢复。
- `DatasetManifest.load()` 除跨 split UID 重复外，还拒绝不同 split 对同一 resolved 音频文件使用重叠的 `[start_ms,end_ms)` 区间；首尾相接的非重叠片段仍允许。speaker overlap 未在缺少显式 speaker-independent split policy 时武断判错。
- 离线推理在 softmax/聚合前拒绝 NaN/Inf logits，避免生成非有限 confidence/probabilities。
...
版本语义复核：strict review 明确指出本轮公开导入面发生 breaking change，继续把同一 API 面标为 `0.2.0` 会混淆发布边界。因此待发布版本统一提升为 `0.3.0`：`pyproject.toml`、`ser_lib/_version.py`、`docs/API_REFERENCE.md` 与 `CHANGELOG.md` 已同步；Stage 01 的 pre-refactor snapshot 继续保留历史 `0.2.0`，不改写 golden evidence。持久化 schema 版本不因 Python API minor bump 被伪造升级。
...
- Strict-review remediation 中间验收 HEAD `b684f90bcc5183c334e382afa888bbd390fb80c0` 对应 CI #458 / run `34477790449`：12/12 jobs 全部 `completed/success`。后续又继续补齐高层流式评估、指标口径、callback cleanup、跨 split 音频片段泄漏门禁及 0.3.0 发布语义，因此 #458 同样只保留为中间证据。
- `0.3.0` bump 后 CI 曾暴露 4 条把“当前运行时版本”硬编码为 `0.2.0` 的 stale assertions；修复只把当前版本断言改为统一版本源，并继续保留 pre-refactor snapshot 的历史 `0.2.0`，没有修改 golden snapshot。
