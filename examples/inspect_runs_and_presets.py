...
def main() -> None:
    ...
    payload: dict[str, object] = {
        ...
        "runtime": get_runtime_capabilities().to_dict(),
    }

    if args.training_run:
        payload["training_run"] = load_training_record(args.training_run).to_dict()
        payload["training_history"] = load_training_history(args.training_run).to_dict()
    if args.evaluation_run:
        run = load_evaluation_record(args.evaluation_run)
