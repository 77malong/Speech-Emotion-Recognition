...
def _fixture(root: Path) -> None:
    ...
    actors = [("1001", "Male"), ("1002", "Male"), ("1003", "Male"), ("1004", "Female"), ("1005", "Female"), ("1006", "Female")]
    with (root / "VideoDemographics.csv").open("w", encoding="utf-8", newline="") as stream:
        ...
        for actor, gender in actors:
            writer.writerow({"ActorID": actor, "Age": 30, "Sex": gender, "Race": "Unknown", "Ethnicity": "Unknown"})
...
def test_crema_d_can_scan_without_demographics(tmp_path: Path) -> None:
    ...
    preview = CremaDImporter().scan(source, {})
    assert preview.ok and len(preview.records) == 36
    assert any("人口统计" in item.message for item in preview.diagnostics if item.severity == "warning")
...
def test_crema_d_rejects_invalid_filename(tmp_path: Path) -> None:
    ...
    assert any(item.path == str(invalid) and item.stage == "filename" for item in preview.diagnostics)
