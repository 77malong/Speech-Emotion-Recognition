...
def _write_wav(path: Path, frequency: float) -> None:
    ...
    for index in range(1600):
        frames.extend(struct.pack("<h", int(6000 * math.sin(2 * math.pi * frequency * index / 16000))))
...
def test_dataset_cli_scan_import_validate_and_stats(tmp_path: Path, capsys):
    ...
    assert main(["dataset", "scan", "--importer", "folder", "--source", str(source), "--params", params, "--json"]) == EXIT_OK
    scan = json.loads(capsys.readouterr().out)
    assert scan["num_records"] == 2
    assert scan["num_errors"] == 0
    assert scan["num_diagnostics"] == 0

    destination = tmp_path / "standard"
    assert main(["dataset", "import", "--importer", "folder", "--source", str(source), "--destination", str(destination), "--params", params, "--json"]) == EXIT_OK
    imported = json.loads(capsys.readouterr().out)
    manifest = Path(imported["manifest"])
    assert manifest.is_file()
    assert imported["stats"]["total"] == 2

    assert main(["dataset", "validate", str(manifest), "--split", "default", "--check-files", "--json"]) == EXIT_OK
    validated = json.loads(capsys.readouterr().out)
    assert validated["valid"] is True
    assert validated["selected_count"] == 2

    assert main(["dataset", "stats", str(manifest), "--split", "default", "--json"]) == EXIT_OK
...
labels:
  0: {en: neutral}
""", encoding="utf-8")
    assert main(["dataset", "validate", str(manifest), "--check-files", "--json"]) == EXIT_DATA
    payload = json.loads(capsys.readouterr().out)
...
def test_cli_configuration_error_has_stable_exit_code(tmp_path: Path, capsys):
    assert main(["dataset", "scan", "--importer", "folder", "--source", str(tmp_path), "--params", "not-json"]) == EXIT_CONFIG
    assert "配置错误" in capsys.readouterr().err


def test_module_entrypoint_reports_version():
    completed = subprocess.run([sys.executable, "-m", "ser_lib.cli", "--version"], check=False, capture_output=True, text=True)
...
def test_train_cli_dispatches_validated_options(monkeypatch, tmp_path: Path, capsys):
    ...
    assert main(["train", str(tmp_path / "experiment.yaml"), "--split", "training", "--batch-size", "8", "--workers", "2", "--json"]) == EXIT_OK
...
def test_predict_cli_dispatches_source_policy(monkeypatch, tmp_path: Path, capsys):
    ...
    assert main(["predict", str(tmp_path / "model"), str(tmp_path / "audio"), "--output", str(tmp_path / "predictions.jsonl"), "--batch-size", "4", "--keep-going", "--no-recursive", "--json"]) == EXIT_OK
...
def test_artifact_export_loads_model_card(monkeypatch, tmp_path: Path, capsys):
    ...
    assert main(["artifact", "export", "--config", str(tmp_path / "experiment.yaml"), "--checkpoint", str(tmp_path / "epoch.pt"), "--destination", str(tmp_path / "artifact"), "--model-card", str(card), "--json"]) == EXIT_OK
...
""", encoding="utf-8")
    ...
    logged = [json.loads(line) for line in Path(result["metrics_log"]).read_text(encoding="utf-8").splitlines()]
