...
def test_ravdess_scan_parses_official_filename_fields(tmp_path: Path):
    ...
    assert any("CC BY-NC-SA 4.0" in item.message for item in preview.diagnostics if item.severity == "warning")
...
def test_ravdess_filter_malformed_and_convert(tmp_path: Path):
    ...
    assert next(item for item in preview.diagnostics if item.severity == "error").stage == "filename"
...
def test_ravdess_rejects_out_of_range_official_codes(tmp_path: Path):
    ...
    preview = RavdessImporter().scan(source, {})
    assert not preview.ok
    assert any("官方范围" in item.message for item in preview.diagnostics if item.severity == "error")
...
def test_manifest_audio_profile_reports_properties_and_failures(tmp_path: Path):
    ...
    (tmp_path / "records.jsonl").write_text(
        "\n".join((
            ...
            json.dumps({"uid": "stereo", "audio_path": "stereo.wav", "label": 1, "start_ms": 50, "end_ms": 150}),
...
def test_repeatable_benchmark_result_round_trip_and_comparison(tmp_path: Path):
    ...
    baseline = BenchmarkResult("forward", 10, 2, 1.0, 2.0, 1.0, 100, environment={"machine": "same"})
    current = BenchmarkResult("forward", 10, 2, 1.05, 2.5, 1.0, 105, environment={"machine": "same"})
