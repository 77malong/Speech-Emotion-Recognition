...
def test_package_coverage_normalizes_windows_paths():
    assert package_coverage(_report(), "ser_lib/foundation/") == 80


def test_coverage_policy_reports_under_threshold(capsys):
    failures = check_report(_report(7), {"ser_lib/foundation/": 75})
    assert failures == ["ser_lib/foundation/ 70.00% < 75.00%"]
