"""集中、显式的持久化 schema migration 基础设施。"""
...
class MigrationRegistry:
    """按 domain/from_version 注册并顺序执行 migration。"""
    ...
    def migrate(
        ...
        *,
        ...
        current = _schema_version(payload, domain=domain)
        ...
        while current < target_version:
            ...
            actual = _schema_version(migrated, domain=domain)
...
def validate_schema_version(
    ...
    *,
    ...
    """集中校验一个 payload 是否属于显式支持的 schema 版本。"""
...
def register_schema_migration(
    ...
    return _default_registry.register(domain, from_version, to_version, function)


def list_schema_migrations(domain: str | None = None) -> tuple[SchemaMigration, ...]:
    return _default_registry.registered(domain)
...
def migrate_schema_payload(
    ...
    *,
    ...
    return _default_registry.migrate(domain, payload, target_version=target_version)
