"""训练与评估运行记录的 read-time version gate。"""
...
def migrate_engine_payload(
    domain: str,
    payload: Mapping[str, Any],
    *,
    target_version: int,
) -> dict[str, Any]:
    """迁移 engine 域 payload；当前真实运行记录均为 v1。"""
    if domain not in _ENGINE_DOMAINS:
        raise ValueError(f"未知 engine migration domain: {domain!r}")
    if target_version < 1:
...
__all__ = ["migrate_engine_payload"]
